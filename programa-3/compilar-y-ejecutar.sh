...
#
# Script que realiza la compilación y ejecución del programa 3 de la materia de Álgebra Lineal.
#
...
g++ *.cpp -o Programa3.out -std=c++17 && echo "¡Compilado Exitosamente! Ejecutando binario de forma interactiva..." && ./Programa3.out;
else
# Esta sección realiza la compilación y ejecuta el programa con las entradas en los archivos *.txt
g++ *.cpp -o Programa3.out -std=c++17 && echo "¡Compilado Exitosamente! Ejecutando binario con archivos de entrada *txt...";
...
do
  ...
  ./Programa3.out < "$i" >> "./ejecuciones/resultados-ejecucion-${fechaHoy}.txt";
done
fi
