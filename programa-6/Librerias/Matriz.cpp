...
void Matriz<P>::imprimirEnRango(int fila_final, int columna_final)
{
  ...
  // Calculamos el número máximo de caracteres necesarios en los elementos de nuestra matriz.
  int caracteres = this->maximoNumeroCaracteres();
  ...
  for (int fila_actual = 0; fila_actual < fila_final; fila_actual++)
  {
    ...
    {
      // Imprimimos el valor.
      std::cout << std::setw(caracteres) << this->matriz_vector[fila_actual][columna_actual] << std::setw(1) << " ";
    }
...
/**
 * Convertir En Cadena
 * Implementación de la función std::to_string que elimina los ceros al final
 * del número y el punto decimal en caso de que sea un valor entero.
 * @param t El valor a convertir a string.
 * @return Un string con el valor.
 */
template <typename T>
std::string convertirEnCadena(const T &t)
{
...
/**
 * Máximo Número Caracteres
 * Calcula el máximo número de caracteres para imprimir los elementos de la matriz
 * actual.
 * @return Un entero con el valor máximo de caracteres para los elementos de la matriz.
 */
template <class P>
int Matriz<P>::maximoNumeroCaracteres()
{
  // Variables locales.
  std::string temporal;   // Para operaciones temporales.
  int longitudMaxima = 0; // Para almacenar la longitud máxima vista hasta ahora.

  for (int filaActual = 0; filaActual < this->obtenerNumeroFilas(); filaActual++)
  {
    for (int columnaActual = 0; columnaActual < this->obtenerNumeroColumnas(); columnaActual++)
    {
      // Convertimos el elemento en un string.
      temporal = convertirEnCadena(this->obtenerElemento(filaActual, columnaActual));

      // Verificamos si la longitud es mayor a la del elemento máximo.
      if (temporal.length() > longitudMaxima)
      {
        // Si fue mayor, lo guardamos.
        longitudMaxima = temporal.length();
      }
    }
  }

  // Regresamos la longitud máxima obtenida.
