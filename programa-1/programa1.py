...
#
# Link de repositorio: https://github.com/humbertowoody/algebra-lineal-escom/tree/main/programa-1
#
...
print("Este programa puede leer ciertas expresiones matemáticas como entradas.")
print("Algunas son: np.sqrt(), fracciones y potencias.")
print()

# Leemos la primer ecuación.
print("Leyendo datos para la primer ecuación.")
x1 = float(eval(input("Ingresa el valor para x1: ")))
y1 = float(eval(input("Ingresa el valor para y1: ")))
b1 = float(eval(input("Ingresa el valor para el término independiente 1: ")))
print()

# Leemos la segunda ecuación.
print("Leyendo datos para la segunda ecuación.")
x2 = float(eval(input("Ingresa el valor para x2: ")))
y2 = float(eval(input("Ingresa el valor para y2: ")))
b2 = float(eval(input("Ingresa el valor para el término independiente 2: ")))
print()

# Generamos los labels para las ecuaciones.
label_ecuacion_1 = "(" + str(x1) + ")x + (" + str(y1) + ")y = " + str(b1)
label_ecuacion_1_para_y = "y = (" + str(b1) + \
    " - (" + str(x1) + ")x) / " + str(y1)
label_ecuacion_2 = "(" + str(x2) + ")x + (" + str(y2) + ")y = " + str(b2)
label_ecuacion_2_para_y = "y = (" + str(b2) + \
    " - (" + str(x2) + ")x) / " + str(y2)
...
matriz_diagonalizada[1][0] = (
    matriz_diagonalizada[0][0] * temporal) + matriz_extendida[1][0]
matriz_diagonalizada[1][1] = (
    matriz_diagonalizada[0][1] * temporal) + matriz_extendida[1][1]
matriz_diagonalizada[1][2] = (
    matriz_diagonalizada[0][2] * temporal) + matriz_extendida[1][2]
...
if ultimo_valor_diagonal == 0 and b2_diagonal != 0:
    print("  ~ El sistema no tiene solución (A_{22}=" + str(ultimo_valor_diagonal) +
          "; b_{2}=" + str(b2_diagonal) + "; A_{22} es cero y b2 es distinto a 0).")
    pass
# Caso dónde el sistema tiene una solución puntual.
elif ultimo_valor_diagonal != 0:
    print("  ~ El sistema tiene solución única (A_{22}=" + str(
        ultimo_valor_diagonal) + " no es igual a cero).")

    # Usamos coeficientes de matriz diagonalizada.
    solucion_y = b2_diagonal / ultimo_valor_diagonal

    # Sustituímos en la primer ecuación.
    solucion_x = (((solucion_y * y1) * -1) + b1) / x1
